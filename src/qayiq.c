...
int main(int argc, char **argv) {
  ...
  int container_pid = 0;
  // socket pair used for communication between qayiq and container
  int sockets[2] = {0};
  ...
  // Check if qayiq is running as root
  if (geteuid() != 0) {
    log_warn("qayiq should be run as root");
  }

  // Set hostname for the container to "qayiqcontainer"
  config.hostname = "qayiqcontainer";
  ...
  // Prepare cgroups for the process (the container is a child process of qayiq)
  ...
  // Qayiq configures the user namespace for the container
...
exit:
  ...
  log_info("Thank you for using Qayiq!");
  return exitcode;
}
