...
QAYIQ := qayiq
QAYIQ_ARGS_0 := --help
QAYIQ_ARGS_1 := -u 0 -m / -c "/bin/bash -c" -a exit
QAYIQ_ARGS_2 := -u 0 -m / -c "/bin/bash -c" -a exit -v
...
# Qayiq object files
OBJS := $(QAYIQ).o cgroups.o container.o mount.o sec.o user.o $(LIB_ARGTABLE_NAME).o $(LIB_LOG_NAME).o
...
# Build qayiq executable
$(QAYIQ): format lint dir $(OBJS)
	$(CC) $(CFLAGS) $(LFLAGS) -o $(BIN_DIR)/$(QAYIQ) $(foreach file,$(OBJS),$(BUILD_DIR)/$(file))
...
# Run CUnit tests
test: dir
	@$(CC) $(CFLAGS) -lcunit -o $(BIN_DIR)/$(QAYIQ)_test $(TESTS_DIR)/$(QAYIQ)_test.c
	@$(BIN_DIR)/$(QAYIQ)_test
...
check: $(QAYIQ)
	@sudo valgrind -s --leak-check=full --show-leak-kinds=all $(BIN_DIR)/$(QAYIQ) $(QAYIQ_ARGS_0)
	@sudo valgrind -s --leak-check=full --show-leak-kinds=all $(BIN_DIR)/$(QAYIQ) $(QAYIQ_ARGS_1)
	@sudo valgrind -s --leak-check=full --show-leak-kinds=all $(BIN_DIR)/$(QAYIQ) $(QAYIQ_ARGS_2)
